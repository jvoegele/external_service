defmodule ExternalServiceTest do
  ...
  describe "call" do
    ...
    test "returns fuse_blown when the fuse is blown by retries" do
      res = ExternalService.call(@fuse_name, @retry_opts, fn() ->
        :retry
      end)
    ...
    test "returns :error when retries are exhausted with :retry" do
      res = ExternalService.call(@fuse_name, @expiring_retry_options, fn() ->
        :retry
      end)
    ...
    test "returns :error when retries are exhausted by an exception" do
    ...
    test "returns :error when not retriable" do
      res = ExternalService.call(@fuse_name, @retry_opts, fn() ->
        {:error, "reason"}
      end)
  ...
  describe "call_async_stream" do
    ...
    test "with no options" do
      results =
        ...
        |> Enum.to_list()
      assert [{:ok, _}, {:ok, _}, {:ok, {:error, {:fuse_blown, :"test-fuse"}}}, {:ok, _},
             {:ok, _}, {:ok, {:error, {:fuse_blown, :"test-fuse"}}}] = results
    end
