# Compiled only when the optional `:decorator` dependency is available. Consumers
# that don't add `:decorator` to their deps simply won't have this module, and the
# rest of ExternalService never references `Decorator`, so the dependency stays
# truly optional.
if Code.ensure_loaded?(Decorator.Define) do
  defmodule ExternalService.Decorator do
    ...
    >
    > `ExternalService.Decorator` exists only when the optional
    > [`:decorator`](https://hex.pm/packages/decorator) dependency is present. Add
    > it to your application's deps to use this module:
    >
    > ```elixir
    > {:decorator, "~> 1.4"}
    > ```
